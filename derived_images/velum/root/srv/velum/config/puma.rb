...
threads 1, ENV["VELUM_PUMA_MAX_THREADS"] || 1
workers ENV["VELUM_PUMA_WORKERS"] || 1
...
if ENV["VELUM_PUMA_HOST"]
  if ENV["VELUM_PUMA_TLS_KEY"]
    host, port = ENV["VELUM_PUMA_HOST"].split(":")
    port ||= "3000"
    ssl_bind host, port,
             key:  ENV["VELUM_PUMA_TLS_KEY"],
             cert: ENV["VELUM_PUMA_TLS_CERT"]
  else
    bind "tcp://#{ENV["VELUM_PUMA_HOST"]}"
  end
else
...
if ENV["VELUM_PUMA_DAEMONIZE"] == "yes"
